...
public class TileBag implements Serializable {
	...
	*/
	...
	/*
	 * Property holding all Tile objects generated. Note: A size of 101 was
	 * selected to ensure all 100 Tile objects are available while preventing an
	 ...
	 */
	public TileBag() {
		// populate the tile_distribution object with letter/letter distribution
		// pairs.
		...
		while (this.tile_distribution.size() > 0) {
			...
			// Gets the next random integer, less than 27, in the random
			// sequence.
			int temp = this.random.nextInt(27);

			// A letter is selected from the letters array using the random
			// digit.
			...
			if (this.tile_distribution.containsKey(temp2)) {
				...
				// and add it to the tile_bag property to enable random
				// selection.
				for (int counter = 0; counter < this.tile_distribution.get(temp2); counter++) {
					this.tile_bag.add(new Tile(temp2));
				}

				// Removes the letter from the tile_distribution because it's
				// number
